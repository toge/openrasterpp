...
auto generate_node_xml(Node const& node, int indent_level, std::ostringstream& ss) -> std::string {
  ...
  if (node.type == Node::Type::Layer) {
    xml += "<layer name='" + name + "' src='data/" + name + ".png' x='" +
      std::to_string(node.x) + "' y='" + std::to_string(node.y) +
      "' visibility='" + vis + "' opacity='" + opacity_str +
      "' composite-op='" + mode + "'/>\n";
    return xml;
  }
...
auto ArchiveXmlProvider::open_archive(std::string_view path, ArchiveMode mode) -> std::expected<void, Error> {
  if (mode == ArchiveMode::Read) {
    ...
    if (!zip_) {
      return make_unexpected(Error::Code::ZipCreateFailed, path);
    }
  }
  return {};
}
...
auto ArchiveXmlProvider::read_entry(std::string_view path) -> std::expected<std::vector<uint8_t>, Error> {
  ...
  if (unzGetCurrentFileInfo64(unzip_, &info, nullptr, 0, nullptr, 0, nullptr, 0) != UNZ_OK) {
    return make_unexpected(Error::Code::ZipReadFailed, path);
  }
...
auto ArchiveXmlProvider::write_entry(std::string_view path, std::span<const uint8_t> data, CompressionLevel level)
  ...
  if (zipOpenNewFileInZip(
    ...
    return make_unexpected(Error::Code::ZipAddFileFailed, path);
  }
