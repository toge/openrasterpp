...
if [ -e "conanfile.py" ]; then
    ...
    cmake -B ${BUILD_DIR} -DCMAKE_TOOLCHAIN_FILE=conan_toolchain.cmake -DCMAKE_BUILD_TYPE=Release -DSTATIC=${STATIC} -S .
else
    ...
    cmake -B ${BUILD_DIR} -DCMAKE_TOOLCHAIN_FILE=${VCPKG_ROOT}/scripts/buildsystems/vcpkg.cmake -DCMAKE_BUILD_TYPE=Release -DSTATIC=${STATIC} -S .
fi
