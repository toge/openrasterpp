...
auto make_document() -> ora::OraDocument {
  ...
  auto layer_png = ora::libpng::encode_png(layer);
...
auto with_preview_assets(ora::OraDocument doc) -> ora::OraDocument {
  ...
  REQUIRE(ora::libpng::render_preview_and_thumbnail(valid_doc).has_value());
...
TEST_CASE("libpng backend encodes image buffers", "[libpng-backend]") {
...
TEST_CASE("libpng backend renders preview assets", "[libpng-backend]") {
  auto doc = make_document();

  auto result = ora::libpng::render_preview_and_thumbnail(doc);
...
TEST_CASE("libpng backend round-trips read/write", "[libpng-backend]") {
  auto doc = make_document();
  REQUIRE(ora::libpng::render_preview_and_thumbnail(doc).has_value());

  auto const path = std::filesystem::path{"test-libpng-roundtrip.ora"};
  std::ignore = std::filesystem::remove(path);

  auto write_result = ora::libpng::write(path.string(), doc);
  REQUIRE(write_result.has_value());

  auto read_result = ora::libpng::read(path.string());
...
TEST_CASE("libpng backend rejects invalid layer png bytes during read", "[libpng-backend]") {
  auto doc = with_preview_assets(make_invalid_document());

  auto const path = std::filesystem::path{"test-libpng-invalid-read.ora"};
  std::ignore = std::filesystem::remove(path);

  REQUIRE(ora::libpng::write(path.string(), doc).has_value());

  auto read_result = ora::libpng::read(path.string());

  REQUIRE_FALSE(read_result.has_value());
  CHECK(read_result.error().code == ora::Error::Code::PngDecodeFailed);
  CHECK(read_result.error().message.contains("libpng"));

  std::ignore = std::filesystem::remove(path);
}

TEST_CASE("libpng backend rejects invalid layer png bytes during render", "[libpng-backend]") {
  ...
  CHECK(result.error().message.contains("libpng"));
}
